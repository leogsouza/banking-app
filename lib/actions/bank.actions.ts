  ...
  try {
    ...
    const accounts = await Promise.all(
      banks?.map(async (bank:Bank) => {
        ...
        const institution = await getInstitution({institutionId: accountsResponse.data.item.institution_id!})
        const account = {
...
export const getAccount = async({ appwriteItemId}: getAccountProps) => {
  try {
    ...
    console.error('An error occurrend while getting the account:', error)
  }
}
...
export const getTransactions = async({accessToken}: getTransactionsProps) => {
  ...
  try {
    ...
    console.error('An error ocurred while getting the transactions:', error)
  }
}
